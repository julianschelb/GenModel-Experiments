...
class TFG:
    ...
    __PARAMETERS = {'do_sample': False,
                    ...
                    # changed from 4, None doesn't work, 500 causes issues w vicuna models # TODO
                    'max_length': 100,
                    'min_length': 1,
                    'num_beam_groups': 2,
                    'num_beams': 2,
    ...
    def __init__(self) -> object:
        ...
        self._api_key = False
        self._api_base = None
        self._model_dev = _Tools._split_dev_ver(
            input_string=self._model_name)[0]
        self._model_version = _Tools._split_dev_ver(
            input_string=self._model_name)[1]
        ...
        self.model = T5ForConditionalGeneration.from_pretrained(
            self._model_name, load_in_8bit=self._memory_saver).to(self._device)
    ...
    def __init__(self, model_name: str, api_base: str = None, configuration: dict = __PARAMETERS,
        ...
        if (model_out_of_pipline is not None) and (tokenizer_out_of_pipline is not None):
            ...
            print(
                f'With {self._model_name}, connecting to {self._api_base}. API key = {self._api_key}')
            self._device = None
        else:
            _Tools._gpu_check_and_connect(self)
    ...
    def split_text(self, text, n_template_tokens):
        chunk_size = self.tokenizer.model_max_length - \
            n_template_tokens  # 85 is the lenght of the prompt

        text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=chunk_size, length_function=self.length_function)
...
class Datasets(TFG):
    ...
    def _dataset_creator_from_df(questionnaire_name: str, df: pd.DataFrame, female_ratio) -> pd.DataFrame:
       ...
       'full_prompt', 'generated text', 'scores', 'outputs'],
        ...
        """
        df_names = pd.read_csv('/home/pop536648/datasets/names_new.csv',
                               index_col=0).sample(frac=1)  # change to something accessible
        new_df = pd.DataFrame()
        male, female = train_test_split(
            df_names, test_size=female_ratio, random_state=0)
            ...
            for i in male.iterrows():
                for d in df.iterrows():
                    ...
                    new_df = pd.concat([new_df, pd.DataFrame(
                        temp_dict, index=[0])], ignore_index=True)
...
class _Tools(TFG):
    ...
    def _t5_loader(TFG):
        ...
        """
        TFG.tokenizer = T5Tokenizer.from_pretrained(
            TFG._model_name, use_auth_token=True)
        print(f'{TFG._model_name} - Tokenizer loaded')
        if TFG._connect_to_gpu:
            TFG.model = T5ForConditionalGeneration.from_pretrained(
                TFG._model_name, device_map="auto", load_in_8bit=TFG._memory_saver, use_auth_token=True)
        else:
            ...
            TFG.model = T5ForConditionalGeneration.from_pretrained(
                TFG._model_name, use_auth_token=True,).to(TFG._device)
        print(
            f'T5 loader done, {TFG._model_name} - Model loaded to {TFG._device}')
    ...
    def _ul2_loader(TFG):
        ...
        print(f'{TFG._model_name} - Tokenizer loaded')
        if TFG._connect_to_gpu:
            TFG.model = AutoModelForSeq2SeqLM.from_pretrained(
                "google/flan-ul2", device_map="auto", load_in_8bit=TFG._memory_saver)
        else:
            TFG.model = AutoModelForSeq2SeqLM.from_pretrained(
                "google/flan-ul2", load_in_8bit=TFG._memory_saver).to(TFG._device)
        print(
            f'UL2 loader done, {TFG._model_name} - Model loaded to {TFG._device}')
    ...
    def _vicuna_and_auto_loader(TFG):
        ...
        """
        try:
            TFG.tokenizer = AutoTokenizer.from_pretrained(
                TFG._model_name, use_fast=True, trust_remote_code=True)
        except:
            TFG.tokenizer = AutoTokenizer.from_pretrained(
                TFG._model_name, use_fast=False, trust_remote_code=True)
        print(f'{TFG._model_name} - Tokenizer loaded')
        if TFG._connect_to_gpu:
            TFG.model = TFG.model = AutoModelForCausalLM.from_pretrained(
                TFG._model_name, trust_remote_code=True, device_map="auto", load_in_8bit=TFG._memory_saver)
        else:
            TFG.model = AutoModelForCausalLM.from_pretrained(
                TFG._model_name, trust_remote_code=True, load_in_8bit=TFG._memory_saver).to(TFG._device)
        print(
            f'Auto Loader done, {TFG._model_name} - Model loaded to {TFG._device}')
    ...
    def _supported_model(TFG):
        ...
        if model_name_to_check in TFG._SUPPORTED_MODELS:
            print(
                f'The model {model_name_to_check} is supported by this pipeline')
        else:
            print(
                f'The model {model_name_to_check} is *NOT* supported by this pipeline, please check the following:\n{TFG._SUPPORTED_MODELS}')
...
class _Generation(TFG):
    ...
    def _generation_without_score(TFG, prompt: str) -> str:
        ...
        """
        input_ids = TFG.tokenizer(
            prompt, return_tensors="pt").input_ids.to(TFG._device)
    ...
    def  _generation_with_score_and_split(TFG, prompt: dict) -> tuple:
        if prompt['split_method'] == 'langchain':
            ...
            prompts = TFG.split_text(
                prompt['prompt'][n_template:], prompt['n_template_tokens'])
            a, s = [], [ ]
            for p in prompts:
                partial_prompt = prompt
                partial_prompt['prompt'] = prompt['template'] + \
                    str(p.page_content)
                response = _Generation._generation_with_score(
                    TFG, partial_prompt)
                a.append(response[0])
                s.append(response[1])
            return a, s, True
            ...
            response = _Generation._generation_with_score(
                TFG, prompt, truncation=True)
            return response[0], response[1], True
        else:
            raise ValueError(
                f"split_method must be 'truncated' or 'langchain', not {split_method}")
    ...
    def _generation_with_score(TFG, prompt: dict, truncation=False) -> tuple:
        ...
        """
        input_ids = TFG.tokenizer(
            prompt['prompt'], return_tensors="pt", truncation=truncation).input_ids.to(TFG._device)
        max_tokens = TFG.tokenizer.model_max_length
        prompt['n_template_tokens'] = TFG.tokenizer(
            prompt['template'], return_tensors="pt").input_ids.real.shape[1]

        if input_ids.real.shape[1] > max_tokens:
            return _Generation._generation_with_score_and_split(TFG, prompt)

        output = TFG.model.generate(
            input_ids, **TFG._parameters, max_new_tokens=100)
